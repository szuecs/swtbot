 ...
 *******************************************************************************/
package org.eclipse.swtbot.eclipse.finder.rmi.waits;
...
public class WaitForEditor extends WaitForObjectCondition<IEditorReference> {
	...
	protected List<IEditorReference> findMatches() {
		return UIThreadRunnable.syncExec(SWTUtils.display(),
				new ListResult<IEditorReference>() {
					public List<IEditorReference> run() {
						return new WorkbenchContentsFinder()
								.findEditors(matcher);
					}
				});
	}
