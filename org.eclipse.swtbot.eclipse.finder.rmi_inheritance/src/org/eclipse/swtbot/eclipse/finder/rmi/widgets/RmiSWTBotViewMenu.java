...
/**
 * A RmiSWTBotViewMenu represents a menu item within a view's menu.
 * 
 * @author @author Stephen Paulin &lt;paulin [at] spextreme [dot] com&gt;
 * @version $Id$
 * @since 1.2
 */
public class RmiSWTBotViewMenu extends SWTBotViewMenu {
	 ...
	 * 
	 * @param commandItem
	 *            The command contribution item.
	 * @throws WidgetNotFoundException
	 *             Thrown if both values are <code>null</code>.
	 * @throws AssertionFailedException
	 *             If the contribution item is <code>null</code>.
	 */
	public RmiSWTBotViewMenu(Command commandItem)
			throws WidgetNotFoundException {
		super(commandItem);
		cmdItem = commandItem;
	 ...
	 *            The action contribution item.
	 * @throws WidgetNotFoundException
	 *             Thrown if both values are <code>null</code>.
	 * @throws AssertionFailedException
	 *             If the contribution item is <code>null</code>.
	 */
	public RmiSWTBotViewMenu(ActionContributionItem contributionItem)
			throws WidgetNotFoundException {
		super(contributionItem);
	 ...
	 * 
	 * @throws WidgetNotFoundException
	 *             Thrown if the action or command id are not valid.
	 */
	public void click() throws WidgetNotFoundException {
		if (commandID != null) {
			...
			UIThreadRunnable.asyncExec(new VoidResult() {
				public void run() {
					try {
						menuClickResult = handlerService.executeCommand(
								commandID, null);
					} catch (Exception e) {
			...
			throw new WidgetNotFoundException(
					"There is no action or contribution id to execute."); //$NON-NLS-1$
	}

	/**
	 * After a click completes, this may be use to access the results returned
	 * by the command. If a click had not previously been done then this value
	 * will be <code>null</code>.
	 * 
	 * @return The object data from the click or <code>null</code> if a click
	 *         never occurred.
	 */
	 ...
	 * @throws WidgetNotFoundException
	 *             Thrown if the action is <code>null</code>.
	 */
	...
	public boolean isChecked() {
		...
		// FIXME This needs to find if a contribution item (Command) has been
		// checked...
		return false;
	}
}
