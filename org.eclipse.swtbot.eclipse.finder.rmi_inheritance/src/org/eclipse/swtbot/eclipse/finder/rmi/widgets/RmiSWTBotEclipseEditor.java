...
import org.eclipse.swtbot.eclipse.finder.rmi.exceptions.QuickFixNotFoundException;
...
import org.eclipse.swtbot.swt.finder.rmi.keyboard.Keyboard;
import org.eclipse.swtbot.swt.finder.rmi.keyboard.Keystrokes;
import org.eclipse.swtbot.swt.finder.rmi.matchers.AbstractMatcher;
...
import org.eclipse.swtbot.swt.finder.rmi.utils.Position;
 ...
 * @version $Id$
 */
public class RmiSWTBotEclipseEditor extends SWTBotEclipseEditor {
	 ...
	 * @param bot
	 *            the instance of {@link RmiSWTWorkbenchBot} which will be used
	 *            to drive operations on behalf of this object.
	 * @throws WidgetNotFoundException
	 *             if the widget is <code>null</code> or widget has been
	 *             disposed.
	 * @since 2.0
	 */
	public RmiSWTBotEclipseEditor(IEditorReference editorReference,
	 ...
	 * @deprecated use {@link RmiSWTBotWorkbenchPart#getReference()} instead
	 */
	 ...
	 * @throws WidgetNotFoundException
	 *             if the quickfix could not be found.
	 */
	 ...
	 * 
	 * @param proposalTable
	 *            the table containing the quickfix.
	 * @param proposalText
	 *            the name of the quickfix to apply.
	 */
	private void selectProposal(SWTBotTable proposalTable, String proposalText) {
		log.debug(MessageFormat.format(
				"Trying to select proposal {0}", proposalText)); //$NON-NLS-1$
		if (proposalTable.containsItem(proposalText)) {
			selectProposal(proposalTable, proposalTable.indexOf(proposalText));
			return;
		}
		throw new QuickFixNotFoundException(
				"Quickfix options not found. Giving up."); //$NON-NLS-1$
	}
	 ...
	 *            the table containing the quickfix.
	 * @param proposalIndex
	 *            the index of the quickfix.
	 */
	private void selectProposal(final SWTBotTable proposalTable,
			final int proposalIndex) {
		log.debug(MessageFormat.format(
				"Trying to select proposal with index {0}", proposalIndex)); //$NON-NLS-1$
	 ...
	 * 
	 * @param proposalShell
	 *            the shell containing the quickfixes.
	 * @return the table containing the quickfix.
	 */
	private SWTBotTable getProposalTable() {
		log.debug("Finding table containing proposals.");
		try {
			Table table = bot.widget(widgetOfType(Table.class),
					activatePopupShell().widget);
			SWTBotTable swtBotTable = new SWTBotTable(table);
			log.debug(MessageFormat.format(
					"Found table containing proposals -- {0}",
					getRows(swtBotTable)));
			return swtBotTable;
		} catch (Exception e) {
			throw new QuickFixNotFoundException(
					"Quickfix options not found. Giving up.", e); //$NON-NLS-1$
		}
	}
	 ...
	 *             if the autocomplete shell did not close in time.
	 * @since 1.2
	 */
	 ...
	 * 
	 * @param insertText
	 *            the text to be inserted before activating the auto-complete.
	 * @param proposalText
	 *            the auto-completion proposal to select from the list.
	 */
	 ...
	 * @return the menu
	 * @throws WidgetNotFoundException
	 *             if the menu with the specified text could not be found.
	 ...
	 *            the column number, 0 based.
	 * @param text
	 *            the text to be typed at the specified location
	...
	/**
	 * @param text
	 *            the text to be typed at the location of the caret. *
	 ...
	 *            the character.
	 * @param keyCode
	 *            any special keys (function keys, arrow or navigation keys
	 *            etc.)
	 * @see Event#keyCode
	 * @see Event#character
	 * @see Event#stateMask
	 ...
	 * 
	 * @param modificationKeys
	 *            the combination of {@link SWT#ALT} | {@link SWT#CTRL} |
	 *            {@link SWT#SHIFT} | {@link SWT#COMMAND}.
	 * @param c
	 *            the character.
	 ...
	 *            {@link SWT#SHIFT} | {@link SWT#COMMAND}.
	 * @param keyCode
	 *            the keyCode, these may be special keys like F1-F12, or
	...
	private Matcher<SWTBotTable> tableWithRow(final String itemText) {
		return new AbstractMatcher<SWTBotTable>() {
			...
			public void describeTo(Description description) {
				description.appendText("table with item (")
						.appendText(itemText).appendText(")");
			}
		};
	}
	...
	private WaitForObjectCondition<SWTBotTable> quickFixAppears(
		...
		return new WaitForObjectCondition<SWTBotTable>(tableMatcher) {
			...
			public String getFailureMessage() {
				return "Could not find auto complete proposal using matcher "
						+ matcher;
			}
	...
	private SWTBotShell activatePopupShell() {
		...
		try {
			...
			throw new QuickFixNotFoundException(
					"Quickfix popup not found. Giving up.", e); //$NON-NLS-1$
		}
	}

	private WaitForObjectCondition<SWTBotTable> autoCompleteAppears(
			Matcher<SWTBotTable> tableMatcher) {
		return new WaitForObjectCondition<SWTBotTable>(tableMatcher) {
			protected List<SWTBotTable> findMatches() {
				try {
					...
					if (matcher.matches(autoCompleteTable)) {
						RmiSWTBotEclipseEditor.this.log
								.debug("matched table, returning");
	...
	private void invokeAction(final String actionId) {
		final IAction action = ((ITextEditor) partReference.getEditor(false))
				.getAction(actionId);
		syncExec(new VoidResult() {
			public void run() {
				log.debug(MessageFormat.format("Activating action with id {0}",
						actionId));
				action.run();
			}
		});
