 ...
 * @since 1.2
 */
public class RmiCommandFinder extends CommandFinder {
	/**
	 * The logging instance for this class.
	 */
	private static final Logger log = Logger.getLogger(RmiCommandFinder.class);

	/**
	 * Creates a RmiCommandFinder.
	 */
	public RmiCommandFinder() {
		// Do nothing.
	}
	 ...
	 * 
	 * @param matcher
	 *            the matcher that can match commands.
	 ...
	 * 
	 * @param service
	 *            the {@link ICommandService} instance to use.
	 * @param matcher
	 *            the matcher that can match the command item.
	 * @return The list of {@link Command}s that match the matcher.
	 */
	public List<SWTBotCommand> findCommand(final ICommandService service,
			...
			public List<SWTBotCommand> run() {
				...
				for (int i = 0; i < commands.length; i++)
					try {
						...
						log
								.error(
										"Failed with an exception on the command: " + commands[i].toString(), e); //$NON-NLS-1$
					}
