...
import org.eclipse.swtbot.eclipse.finder.rmi.finders.RmiCommandFinder;
import org.eclipse.swtbot.eclipse.finder.rmi.finders.ViewMenuFinder;
import org.eclipse.swtbot.eclipse.finder.rmi.widgets.utils.PartLabelDescription;
...
import org.eclipse.swtbot.swt.finder.rmi.results.ListResult;
...
import org.eclipse.swtbot.swt.finder.rmi.widgets.SWTBotToolbarPushButton;
...
/**
 * This represents the eclipse {@link IWorkbenchPartReference} item, subclasses
 * must extend this to implement support for various
 * {@link IWorkbenchPartReference}s.
 * 
 ...
 * @since 2.0
 */
public abstract class RmiSWTBotWorkbenchPart<T extends IWorkbenchPartReference>
		extends SWTBotWorkbenchPart {
	...
	protected final RmiSWTWorkbenchBot bot;
	 ...
	 *            the helper bot.
	 */
	public RmiSWTBotWorkbenchPart(T partReference, RmiSWTWorkbenchBot bot) {
	 ...
	 *            the helper bot.
	 * @param description
	 *            the description of the workbench part.
	 */
	public RmiSWTBotWorkbenchPart(T partReference, RmiSWTWorkbenchBot bot,
	...
	public void show() {
		UIThreadRunnable.syncExec(new VoidResult() {
			public void run() {
				try {
					partReference.getPage().activate(
							partReference.getPart(true));
					partReference.getPage().showView(partReference.getId());
				} catch (PartInitException e) {
					throw new RuntimeException(
							"Could not show partReference - " + partReference.getPartName(), e); //$NON-NLS-1$
				}
			}
		});
	}
	...
	/**
	 * Gets a list of all menus within the partReference. This will also include
	 * sub menus.
	 * 
	 * @return The list of menus
	 */
	public List<RmiSWTBotViewMenu> menus() {
		return menuFinder.findMenus((IViewReference) partReference, anything(),
				true);
	}

	/**
	 * Gets a menu item matching the give label within the partReference menu if
	 * one exists.
	 * 
	 * @param label
	 *            The label matching name in the menu.
	 * @return The {@link SWTBotMenu} item.
	 * @throws WidgetNotFoundException
	 *             Thrown if the menu can not be found or if the partReference
	 *             does not contain a menu.
	 */
	public RmiSWTBotViewMenu menu(String label) throws WidgetNotFoundException {
		return menu(label, 0);
	}
	 ...
	 *            The label matching name in the menu.
	 * @param index
	 *            The index of the menu to choose.
	 * @return The {@link SWTBotMenu} item.
	 * @throws WidgetNotFoundException
	 *             Thrown if the menu can not be found or if the partReference
	 *             does not contain a menu.
	 */
	public RmiSWTBotViewMenu menu(String label, int index)
			throws WidgetNotFoundException {
		try {
			List<RmiSWTBotViewMenu> menuItems = menuFinder.findMenus(
					(IViewReference) partReference, withMnemonic(label), true);
			if ((menuItems == null) || (menuItems.size() < 1)) {
				RmiCommandFinder finder = new RmiCommandFinder();
				List<RmiSWTBotCommand> command = finder
						.findCommand(equalTo(label));
				return command.get(index);
			}
			return menuItems.get(index);
		} catch (Exception e) {
			throw new WidgetNotFoundException(
					"Could not find view menu with label " + label + " at index " + index, e); //$NON-NLS-1$ //$NON-NLS-2$
		}
	}
	...
	public List<SWTBotToolbarButton> getToolbarButtons() {
		return UIThreadRunnable.syncExec(new ListResult<SWTBotToolbarButton>() {
			...
			public List<SWTBotToolbarButton> run() {
				PartPane obj = ((WorkbenchPartReference) partReference)
						.getPane();
				...
				for (int i = 0; i < items.length; i++) {
					try {
						...
						log
								.warn(
										"Failed to find widget " + items[i].getText(), e); //$NON-NLS-1$
					}
				}
	 ...
	 * @throws WidgetNotFoundException
	 *             Thrown if the widget was not found matching the given
	 *             tooltip.
	 */
	public SWTBotToolbarDropDownButton toolbarDropDownButton(String tooltip)
		...
		throw new WidgetNotFoundException(
				"Unable to find toolitem with the given tooltip '" + tooltip + "'"); //$NON-NLS-1$ //$NON-NLS-2$
	}
	 ...
	 * @param matcher
	 *            a matcher.
	 * @return a widget within the parent widget that matches the specified
	 *         matcher.
	 */
	 ...
	 *         matcher.
	 */
	protected <S extends Widget> List<? extends S> findWidgets(
			Matcher<S> matcher) {
		Finder finder = bot.getFinder();
		Control control = getControl();
		boolean shouldFindInvisibleControls = finder
		...
		try {
			...
			throw new WidgetNotFoundException(
					"Could not find any control inside the view " + partReference.getPartName(), e); //$NON-NLS-1$
		} finally {
	...
	/**
	 * Returns a SWTBot instance that matches the contents of this workbench
	 * part.
	 * 
	 * @return SWTBot
	 */
	...
	protected void assertActive() {
		Assert.isLegal(isActive(), MessageFormat.format(
				"The workbench part {0}is not active", description));
	}
	...
	/**
	 * The parent widget inside the partReference. If you want to look for a
	 * particular widget within the part, this is a good place to start
	 * searching for the widget.
	 * <p>
	 * <b>NOTE:</b> Clients must ensure that the part is active at the time of
	 * making this call. If the part is not active, then this method will throw
