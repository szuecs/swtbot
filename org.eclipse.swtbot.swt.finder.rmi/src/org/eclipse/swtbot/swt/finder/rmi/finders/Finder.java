 ...
 *******************************************************************************/
package org.eclipse.swtbot.swt.finder.rmi.finders;
...
/**
 * A wrapper around {@link ControlFinder} and {@link MenuFinder} that delegates
 * to either of them.
 * 
...
public class Finder implements Serializable {
	...
	private static final long serialVersionUID = -250347291714271247L;
	/**
	 ...
	 * 
	 * @param controlFinder
	 *            the finder that finds controls.
	 * @param menuFinder
	 *            the finder that finds menus.
	 */
	...
	/**
	 * Establishes the finder from an existing finder (control finder only) and
	 * the given new menu finder.
	 * 
	 * @param finder
	 *            the finder
	 * @param menuFinder
	 ...
	 * If recursive is set, it will attempt to recursively find the controls in
	 * each {@code children} widget if they exist.
	 * 
	 * @param children
	 *            the list of widgets.
	 * @param matcher
	 *            the matcher used to match the widgets.
	 * @param recursive
	 *            if the match should be recursive.
	 * @return all visible widgets in the children that the matcher matches. If
	 *         recursive is <code>true</code> then find the widgets within each
	 *         of the widget.
	 * @see ControlFinder#findControls(List, Matcher, boolean)
	 */
	public <T extends Widget> List<T> findControls(List<Widget> children,
			Matcher<T> matcher, boolean recursive) {
		return controlFinder.findControls(children, matcher, recursive);
	}

	/**
	 * Finds the controls starting with the given parent widget and uses the
	 * given matcher. If recursive is set, it will attempt to find the controls
	 * in each child widget if they exist.
	 * <p>
	 * This method is thread safe.
	 * </p>
	 * 
	 ...
	 * @param recursive
	 *            if the match should be recursive.
	 * @return all visible widgets in the parentWidget that the matcher matches.
	 *         If recursive is <code>true</code> then find the widget within
	 *         each of the parentWidget.
	...
	/**
	 * Finds a menu matching the given item in all available shells. This
	 * searches for menus recursively.
	 * 
	 * @param matcher
	...
	/**
	 * Fins all the menus in the given menu bar matching the given matcher. If
	 ...
	 */
	public List<MenuItem> findMenus(Menu bar, Matcher<MenuItem> matcher,
			boolean recursive) {
		return menuFinder.findMenus(bar, matcher, recursive);
	}

	/**
	 * Finds the menus in the given shell using the given matcher. If recursive
	 * is set, it will attempt to find the controls recursively in each of the
	 * menus it that is found.
	 * 
	 * @param shell
	 *            the shell to probe for menus.
	 * @param matcher
	 *            the matcher that can match menus and menu items.
	 * @param recursive
	 *            if set to true, will find sub-menus as well.
	 ...
	 */
	public List<MenuItem> findMenus(Shell shell, Matcher<MenuItem> matcher,
			boolean recursive) {
	 ...
	 * @see MenuFinder#findMenus(Shell[], Matcher, boolean)
	 */
	public List<MenuItem> findMenus(Shell[] shells, Matcher<MenuItem> matcher,
			boolean recursive) {
		return menuFinder.findMenus(shells, matcher, recursive);
	}

	/**
	 * Checks if this should return items that are not visible when performing
	 * the search for controls.
	 * 
	 * @return <code>true</code> if the finder should return items that are not
	 *         visible. Otherwise <code>false</code> is returned to show no
	 *         items will be returned if they are not visible.
	 * @since 1.0
	 */
	...
	/**
	 * This sets the flag to know if items should be returned if they are not
	 * visible.
	 * 
	 * @param shouldFindInVisibleControls
	 *            <code>true</code> to cause controls that are not visible to be
	 *            found. Use false so that controls that are visible will be
	 *            returned.
	 * @since 1.0
	 */
	public void setShouldFindInvisibleControls(
