...
public class SWTBotList extends AbstractSWTBotControl<List> implements
		Serializable {

	private static final long serialVersionUID = -4616201456763997724L;
	 ...
	 *             disposed.
	 */
	public SWTBotList(List list, SelfDescribing description)
			throws WidgetNotFoundException {
	...
	public void select(final String item) {
		log.debug(MessageFormat.format(
				"Set selection {0} to text {1}", this, item)); //$NON-NLS-1$
		assertEnabled();
	...
	public void select(final int index) {
		log.debug(MessageFormat.format(
				"Set selection {0} to index {1}", this, index)); //$NON-NLS-1$
		assertEnabled();
	 ...
	 * 
	 * @param indices
	 *            the indices to select in the list.
	 */
	public void select(final int[] indices) {
		log
				.debug(MessageFormat
						.format(
								"Set selection {0} to indices {1}]", this, StringUtils.join(indices, ", "))); //$NON-NLS-1$ //$NON-NLS-2$
		assertEnabled();
		asyncExec(new VoidResult() {
			public void run() {
	 ...
	 * 
	 * @param items
	 *            the items to select in the list.
	 */
	public void select(final String[] items) {
		log
				.debug(MessageFormat
						.format(
								"Set selection {0} to items [{1}]", this, StringUtils.join(items, ", "))); //$NON-NLS-1$ //$NON-NLS-2$
		assertEnabled();
		asyncExec(new VoidResult() {
			public void run() {
				widget.deselectAll();
