...
import org.eclipse.swtbot.swt.finder.rmi.results.ArrayResult;
...
public class SWTBotTree extends AbstractSWTBot<Tree> implements Serializable {

	private static final long serialVersionUID = 1805423599800242301L;
	 ...
	 *             disposed.
	 */
	public SWTBotTree(Tree tree, SelfDescribing description)
			throws WidgetNotFoundException {
	...
	public String cell(final int row, final int column) {
		...
		Assert
				.isLegal(
						column < columnCount,
						"The column number (" + column + ") is more than the number of column(" + columnCount //$NON-NLS-1$ //$NON-NLS-2$
		...
		asyncExec(new VoidResult() {
			public void run() {
				...
				if (hasStyle(widget, SWT.MULTI) && items.length > 1)
					log
							.warn("Tree does not support SWT.MULTI, cannot make multiple selections"); //$NON-NLS-1$
		...
		asyncExec(new VoidResult() {
			public void run() {
				log
						.debug(MessageFormat
								.format(
										"Selecting rows [{0}] in tree{1}", StringUtils.join(indices, ", "), this)); //$NON-NLS-1$ //$NON-NLS-2$
	 ...
	 */
	public SWTBotTreeItem expandNode(final String nodeText,
			final boolean recursive) throws WidgetNotFoundException {
		assertEnabled();
		return syncExec(new Result<SWTBotTreeItem>() {
			...
			private void expandNode(SWTBotTreeItem item)
					throws WidgetNotFoundException {
				item.expand();
				if (recursive)
			...
			private void expandTreeItem(TreeItem node)
					throws WidgetNotFoundException {
	...
	// */
	// public SWTBotTreeItem expandNode(final SWTBotTreeItem node, final boolean
	// recursive) throws
	...
	// }
	//
	// private void expandNode(SWTBotTreeItem node) throws
	// WidgetNotFoundException {
	...
	// }
	//
	// private void expandTreeItem(SWTBotTreeItem node) throws
	// WidgetNotFoundException {
	 ...
	 */
	public SWTBotTreeItem getTreeItem(final String nodeText)
			throws WidgetNotFoundException {
		try {
			...
			throw new WidgetNotFoundException(
					"Timed out waiting for tree item " + nodeText, e); //$NON-NLS-1$
		}
	 ...
	 * 
	 * @return the list of all tree items in the tree, or an empty list if there
	 *         are none.
	 * @since 1.0
	 */
