...
public class SWTBotToolbarDropDownButton extends SWTBotToolbarButton {
	...
	private static final long serialVersionUID = -3805441814393743576L;
	 ...
	 *             disposed.
	 */
	public SWTBotToolbarDropDownButton(ToolItem w, SelfDescribing description)
			throws WidgetNotFoundException {
		super(w, description);
		Assert.isTrue(SWTUtils.hasStyle(w, SWT.DROP_DOWN),
				"Expecting a drop down button."); //$NON-NLS-1$
	...
	/**
	 * Finds all the submenus inside this menu item that match the given
	 * matcher.
	 * <p>
	 * <b>NOTE:</b>Invoking this keeps the menu open until you click on it.
	 * </p>
	 ...
	 * @throws WidgetNotFoundException
	 *             if the menuItem could not be found
	 */
	public List<? extends SWTBotMenu> menuItems(Matcher<MenuItem> matcher) {
		...
		try {
			...
			List<MenuItem> findMenus = menuFinder.findMenus(new SWTBot()
					.activeShell().widget, matcher, true);
	...
	private ArrayList<SWTBotMenu> toSWTBotMenuItems(Matcher<?> matcher,
			List<MenuItem> findMenus) {
		ArrayList<SWTBotMenu> result = new ArrayList<SWTBotMenu>(findMenus
				.size());
