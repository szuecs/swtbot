...
import org.eclipse.swtbot.swt.finder.rmi.results.IntResult;
import org.eclipse.swtbot.swt.finder.rmi.results.ListResult;
import org.eclipse.swtbot.swt.finder.rmi.results.Result;
import org.eclipse.swtbot.swt.finder.rmi.results.StringResult;
...
import org.eclipse.swtbot.swt.finder.rmi.utils.StringUtils;
import org.eclipse.swtbot.swt.finder.rmi.utils.TableCollection;
import org.eclipse.swtbot.swt.finder.rmi.utils.TableRow;
...
public class SWTBotTable extends AbstractSWTBot<Table> implements Serializable {

	private static final long serialVersionUID = -8884857762308974221L;
	 ...
	 *             disposed.
	 */
	public SWTBotTable(Table table, SelfDescribing description)
			throws WidgetNotFoundException {
	 ...
	 */
	public SWTBotTableColumn header(final String label)
			throws WidgetNotFoundException {
	...
	public String cell(int row, String columnName) {
		Assert.isLegal(columns().contains(columnName),
				"The column `" + columnName + "' is not found."); //$NON-NLS-1$ //$NON-NLS-2$
	...
	private void assertIsLegalRowIndex(final int rowIndex) {
		Assert.isLegal(rowIndex < rowCount(),
				"The row number: " + rowIndex + " does not exist in the table"); //$NON-NLS-1$ //$NON-NLS-2$
	}
	...
	public void select(final String... items) {
		...
		for (int i = 0; i < items.length; i++) {
			...
			Assert.isLegal(itemIndicies[i] >= 0,
					"Could not find item:" + items[i] + " in table"); //$NON-NLS-1$ //$NON-NLS-2$
		}
		select(itemIndicies);
		notifySelect();
	}
	 ...
	 * @return the index of the specified item in the table, or -1 if the item
	 ...
	 * 
	 * @param item
	 *            the index of the item in the table, or -1 if the item does not
	 *            exist in the table.
	 * @param column
	 *            the column for which to get the index of.
	 * @return the index of the specified item and of the specified column in
	 *         the table.
	 * @since 1.3
	 */
		...
		asyncExec(new VoidResult() {
			public void run() {
				log.debug(MessageFormat
						.format("Unselecting all in {0}", widget)); //$NON-NLS-1$
				widget.deselectAll();
			}
		});
		notifySelect();
	...
	public void select(final int... indices) {
		...
		log
				.debug(MessageFormat
						.format(
								"Selecting rows {0} in table {1}", StringUtils.join(indices, ", "), this)); //$NON-NLS-1$ //$NON-NLS-2$
		unselect();
	...
	private void assertMultiSelect() {
		Assert.isLegal(hasStyle(widget, SWT.MULTI),
				"Table does not support multi selection."); //$NON-NLS-1$
	}
	...
	/**
	 * Click on the table on given cell. This can be used to activate a
	 * cellEditor on a cell.
	 * 
	 * @param row
	 *            the row in the table.
	 * @param column
	...
	public void doubleClick(final int row, final int column) {
		...
		asyncExec(new VoidResult() {
			public void run() {
				TableItem item = widget.getItem(row);
				Rectangle cellBounds = item.getBounds(column);
				// for some reason, it does not work without setting selection
				// first
				widget.setSelection(row);
				doubleClickXY(cellBounds.x + (cellBounds.width / 2),
	...
	protected void assertIsLegalCell(final int row, final int column) {
		...
		Assert
				.isLegal(
						row < rowCount,
						"The row number (" + row + ") is more than the number of rows(" + rowCount + ") in the table."); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		Assert
				.isLegal(
						(column < columnCount)
								|| ((columnCount == 0) && (column == 0)),
						"The column number (" + column //$NON-NLS-1$
	...
	public SWTBotTableItem getTableItem(final String itemText)
		...
		try {
			...
			throw new WidgetNotFoundException(
					"Timed out waiting for table item " + itemText, e); //$NON-NLS-1$
		}
	 ...
	 * @since 2.0
	 */
	public SWTBotTableItem getTableItem(final int row)
			throws WidgetNotFoundException {
		try {
			new SWTBot().waitUntil(new DefaultCondition() {
				public String getFailureMessage() {
			...
			throw new WidgetNotFoundException(
					"Timed out waiting for table item in row " + row, e); //$NON-NLS-1$
		}
