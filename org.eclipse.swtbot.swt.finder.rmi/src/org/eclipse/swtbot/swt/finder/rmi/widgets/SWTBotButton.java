...
public class SWTBotButton extends AbstractSWTBotControl<Button> implements
		Serializable {

	private static final long serialVersionUID = -1736547637128448917L;
	...
	public SWTBotButton(Button button, SelfDescribing description) {
		...
		Assert.isTrue(SWTUtils.hasStyle(button, SWT.PUSH),
				"Expecting a push button."); //$NON-NLS-1$
	}
	...
	public SWTBotButton click() {
		log.debug(MessageFormat.format(
				"Clicking on {0}", SWTUtils.getText(widget))); //$NON-NLS-1$
		assertEnabled();
		...
		notify(SWT.FocusOut);
		log.debug(MessageFormat.format(
				"Clicked on {0}", SWTUtils.getText(widget))); //$NON-NLS-1$
		return this;
	}
