 ...
 *******************************************************************************/
package org.eclipse.swtbot.swt.finder.rmi;

import static org.eclipse.swtbot.swt.finder.rmi.finders.UIThreadRunnable.syncExec;
import static org.eclipse.swtbot.swt.finder.rmi.matchers.WidgetMatcherFactory.allOf;
import static org.eclipse.swtbot.swt.finder.rmi.matchers.WidgetMatcherFactory.widgetOfType;
import static org.eclipse.swtbot.swt.finder.rmi.matchers.WidgetMatcherFactory.withId;
import static org.eclipse.swtbot.swt.finder.rmi.matchers.WidgetMatcherFactory.withMnemonic;
import static org.eclipse.swtbot.swt.finder.rmi.matchers.WidgetMatcherFactory.withText;
import static org.eclipse.swtbot.swt.finder.rmi.matchers.WidgetMatcherFactory.withTooltip;
import static org.eclipse.swtbot.swt.finder.rmi.waits.Conditions.waitForMenu;
import static org.eclipse.swtbot.swt.finder.rmi.waits.Conditions.waitForShell;
import static org.eclipse.swtbot.swt.finder.rmi.waits.Conditions.waitForWidget;
...
import org.eclipse.swtbot.swt.finder.rmi.matchers.WithItem;
...
import org.eclipse.swtbot.swt.finder.rmi.utils.SWTBotPreferences;
...
import org.eclipse.swtbot.swt.finder.rmi.waits.WaitForObjectCondition;
...
abstract class SWTBotFactory {
	...
	/**
	 * the delay between successive polling while waiting for a condition to be
	 * true. Use SWTBotPreferences#DEFAULT_POLL_DELAY
	 */
	@Deprecated
	 ...
	 *            the text on the shell.
	 * @param index
	 *            the index of the shell, in case there are multiple shells with
	 *            the same text.
	 */
	 ...
	 * @param value
	 *            the value of the id
	 * @return a wrapper around a @{link Shell} with the specified value pair
	 *         for its id.
	 * @since 2.0
	 */
	 ...
	 * @param value
	 *            the value of the id
	 * @return a wrapper around a @{link Shell} with the specified key/value
	 *         pair for its id.
	 */
	 ...
	 *            the index of the shell, in case there are multiple shells with
	 *            the same text.
	 * @return a wrapper around a @{link Shell} with the specified value for its
	 *         id.
	 * @since 2.0
	 */
	...
	/**
	 * @param shell
	 *            the shell to search for the menu.
	 * @param matcher
	 *            the matcher used to find the menu.
	 * @param index
	 *            the index of the menu, in case there are multiple menus with
	 *            the same text.
	 * @return a menu item that matches the specified text.
	...
	public SWTBotMenu menu(SWTBotShell shell, Matcher<MenuItem> matcher,
			int index) {
		WaitForObjectCondition<MenuItem> waitForMenu = waitForMenu(shell,
				matcher);
	 ...
	 *            the parent widget to search for a given widget.
	 * @param index
	 *            the index of the widget, incase the matcher finds multiple
	 *            widgets
	 * @return the first widget that matchs the matcher.
	 */
	public <T extends Widget> T widget(Matcher<T> matcher, Widget parentWidget,
	 ...
	 * 
	 * @param condition
	 *            the {@link ICondition} to be evaluated.
	 * @throws TimeoutException
	 *             if the condition does not evaluate to true after
	 *             {@link SWTBotPreferences#TIMEOUT} milliseconds.
	 * @since 1.2
	 */
		...
		while (true) {
			...
			if (System.currentTimeMillis() > limit)
				throw new TimeoutException(
						"Timeout after: " + timeout + " ms.: " + condition.getFailureMessage()); //$NON-NLS-1$ //$NON-NLS-2$
		}
	}
	 ...
	 *            the {@link ICondition} to be evaluated.
	 * @throws TimeoutException
	 *             if the condition does not evaluate to false after
	 *             {@link SWTBotPreferences#TIMEOUT} milliseconds.
	 * @since 2.0
	 */
	 ...
	 *            the timeout.
	 * @throws TimeoutException
	 *             if the condition does not evaluate to false after timeout
	 *             milliseconds.
	 * @since 2.0
	 */
	...
	/**
	 * Waits while the condition is true, until the timeout is reached. The
	 * interval is the delay between evaluating the condition after it has
	 * succeed.
	 * 
	 * @param condition
	 ...
	 * 
	 * @param millis
	 *            the time in milliseconds for which to sleep.
	 */
	...
	/**
	 * Returns the control which currently has keyboard focus, or
	 * <code>null</code> if keyboard events are not currently going to any of
	 * the controls built by the currently running application.
	 * 
	 ...
	 * @return the first {@link SWTBotTrayItem} with the specified
	 *         <code>tooltip</code>.
	 */
	...
	public SWTBotTrayItem trayItemWithTooltip(String tooltip, int index) {
		java.util.List<SWTBotTrayItem> items = trayItems(allOf(
				widgetOfType(TrayItem.class), withTooltip(tooltip)));
		if (items.isEmpty() || items.size() <= index)
			throw new WidgetNotFoundException(
					"Widget does not contain an item at index " + index);
	...
	public List<SWTBotTrayItem> trayItems(Matcher<?> matcher) {
		...
		WaitForObjectCondition<?> waitForWidget = waitForWidget(itemMatcher,
				systemTray());
		waitUntilWidgetAppears(waitForWidget);
	 ...
	 * <p>
	 * This is a convenience api for performing a particular operation with a
	 * different timeout:
	 * </p>
	 * 
	 * <pre>
