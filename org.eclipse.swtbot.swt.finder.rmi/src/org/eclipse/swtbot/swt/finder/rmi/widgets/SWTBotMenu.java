 ...
 * @version $Id$
 */
public class SWTBotMenu extends AbstractSWTBot<MenuItem> implements
		Serializable {

	private static final long serialVersionUID = 10729547553423951L;
	 ...
	 *             disposed.
	 */
	public SWTBotMenu(MenuItem w, SelfDescribing description)
			throws WidgetNotFoundException {
		super(w, description);
	}
	 ...
	 * 
	 * @param menuName
	 *            the name of the menu item that is to be found
	 * @return the first menu that matches the menuName
	 * @throws WidgetNotFoundException
	 *             if the widget is not found.
	 */
	public SWTBotMenu menu(final String menuName)
			throws WidgetNotFoundException {
		...
		MenuItem menuItem = syncExec(new WidgetResult<MenuItem>() {
			public MenuItem run() {
				...
				List<MenuItem> menus = new MenuFinder().findMenus(bar,
						withMnemonic, true);
				if (!menus.isEmpty())
					return menus.get(0);
				return null;
	 ...
	 * 
	 * @return <code>true</code> if the menu is checked, <code>false</code>
	 *         otherwise.
