 ...
 * @version $Id$
 */
@SWTBotWidget(clasz = Combo.class, preferredName = "comboBox", referenceBy = {
		ReferenceBy.LABEL, ReferenceBy.TEXT })
public class SWTBotCombo extends AbstractSWTBotControl<Combo> implements
		Serializable {

	private static final long serialVersionUID = 7426308889522520234L;
	 ...
	 * @since 1.0
	 */
	public SWTBotCombo(Combo w, SelfDescribing description)
			throws WidgetNotFoundException {
		super(w, description);
	}
	...
	public void setSelection(final String text) {
		log.debug(MessageFormat.format(
				"Setting selection on {0} to {1}", this, text)); //$NON-NLS-1$
		assertEnabled();
		_setSelection(text);
		notify(SWT.Selection);
		log.debug(MessageFormat.format(
				"Set selection on {0} to {1}", this, text)); //$NON-NLS-1$
	}
	...
	public void setText(final String text) {
		log.debug(MessageFormat
				.format("Setting text on {0} to {1}", this, text)); //$NON-NLS-1$
		assertEnabled();
