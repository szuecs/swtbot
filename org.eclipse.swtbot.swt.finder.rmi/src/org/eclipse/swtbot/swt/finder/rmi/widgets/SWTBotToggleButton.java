...
public class SWTBotToggleButton extends AbstractSWTBot<Button> implements
		Serializable {

	private static final long serialVersionUID = -8497131509030951773L;
	 ...
	 * @since 1.0
	 */
	public SWTBotToggleButton(Button w, SelfDescribing description)
			throws WidgetNotFoundException {
		super(w, description);
		Assert.isTrue(SWTUtils.hasStyle(w, SWT.TOGGLE),
				"Expecting a toggle button."); //$NON-NLS-1$
	}
	...
	public void unpress() {
		...
		if (!isPressed()) {
			log
					.debug(MessageFormat
							.format(
									"Widget {0} already deselected, not deselecting again.", this)); //$NON-NLS-1$
			return;
		}
	...
	public void press() {
		...
		if (isPressed()) {
			log.debug(MessageFormat.format(
					"Widget {0} already selected, not selecting again.", this)); //$NON-NLS-1$
			return;
		}
		asyncExec(new VoidResult() {
			public void run() {
		...
		asyncExec(new VoidResult() {
			public void run() {
				log
						.debug(MessageFormat
								.format(
										"Toggling state on {0}. Setting state to {1}", widget, (!widget.getSelection() ? "selected" //$NON-NLS-1$ //$NON-NLS-2$
	 ...
	 * 
	 * @return <code>true</code> if the toggle button is pressed. Otherwise
	 *         <code>false</code>.
	 */
