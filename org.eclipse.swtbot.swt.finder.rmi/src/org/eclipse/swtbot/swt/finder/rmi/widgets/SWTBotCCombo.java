 ...
 * @since 1.0
 */
@SWTBotWidget(clasz = CCombo.class, preferredName = "ccomboBox", referenceBy = {
		ReferenceBy.TEXT, ReferenceBy.LABEL })
public class SWTBotCCombo extends AbstractSWTBotControl<CCombo> implements
		Serializable {

	private static final long serialVersionUID = -541286545460870028L;
	 ...
	 *             disposed.
	 */
	public SWTBotCCombo(CCombo w, SelfDescribing description)
			throws WidgetNotFoundException {
		super(w, description);
	}
	...
	public void setText(final String text) {
		log.debug(MessageFormat.format(
				"Setting text on widget {0} to {1}", this, text)); //$NON-NLS-1$
		assertEnabled();
	...
	public void setSelection(final String text) {
		log.debug(MessageFormat.format(
				"Setting selection on {0} to {1}", widget, text)); //$NON-NLS-1$
		_setSelection(text);
		notify(SWT.Selection);
		log.debug(MessageFormat.format(
				"Set selection on {0} to {1}", widget, text)); //$NON-NLS-1$
	}
	...
	private void _setSelection(final String text) {
		...
		if (indexOf == -1)
			throw new RuntimeException(
					"Item `" + text + "' not found in combo box."); //$NON-NLS-1$ //$NON-NLS-2$
	 ...
	 * 
	 * @return the current selection in the combo box or null if no item is
	 *         selected.
	 */
	...
	public void setSelection(final int index) {
		...
		if (index > itemCount)
			throw new RuntimeException(
					"The index (" + index + ") is more than the number of items (" + itemCount + ") in the combo."); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
