...
import org.eclipse.swtbot.eclipse.finder.rmi.finders.CommandFinder;
...
public abstract class SWTBotWorkbenchPart<T extends IWorkbenchPartReference> {
	 ...
	 *            the helper bot.
	 */
	public SWTBotWorkbenchPart(T partReference, RmiSWTWorkbenchBot bot) {
	 ...
	 *             does not contain a menu.
	 */
	public SWTBotViewMenu menu(String label, int index)
			throws WidgetNotFoundException {
		try {
			List<SWTBotViewMenu> menuItems = menuFinder.findMenus(
			...
			if ((menuItems == null) || (menuItems.size() < 1)) {
				...
				List<SWTBotCommand> command = finder
