...
import org.eclipse.swtbot.eclipse.finder.rmi.waits.WaitForView;
import org.eclipse.swtbot.eclipse.finder.rmi.widgets.SWTBotEclipseEditor;
...
/**
 * This extends the {@link SWTWorkbenchBot} and adds specific capabilities for
 * writing Eclipse IDE tests.
 * 
 * @author Ketan Padegaonkar &lt;KetanPadegaonkar [at] gmail [dot] com&gt;
 * @author Ralf Ebert www.ralfebert.de (bug 271630)
 * @version $Id$
 * @deprecated use {@link SWTWorkbenchBot}. This will be removed from future
 *             releases.
 */
public class SWTEclipseBot extends SWTWorkbenchBot {
	 ...
	 */
	public SWTBotEclipseEditor editor(String fileName)
			throws WidgetNotFoundException {
	...
	public SWTBotEclipseEditor editor(String fileName, int index)
			throws WidgetNotFoundException {
		...
		return new SWTBotEclipseEditor(waitForEditor.get(index),
				new RmiSWTWorkbenchBot(this));
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public SWTBotView view(String label, int index)
			throws WidgetNotFoundException {
		...
		return new SWTBotView(waitForView.get(index), new RmiSWTWorkbenchBot(
				this));
	}
	...
	public List<SWTBotEclipseEditor> editors() throws WidgetNotFoundException {
		...
		List<SWTBotEclipseEditor> result = new ArrayList<SWTBotEclipseEditor>(
				editors.size());

		for (IWorkbenchPartReference editor : editors) {
			result.add(new SWTBotEclipseEditor((IEditorReference) editor,
					new RmiSWTWorkbenchBot(this)));
		}
		return result;
	}
	...
	public List<SWTBotView> views() throws WidgetNotFoundException {
		...
		for (IWorkbenchPartReference editor : editors) {
			result.add(new SWTBotView((IViewReference) editor,
					new RmiSWTWorkbenchBot(this)));
		}
		return result;
	}
